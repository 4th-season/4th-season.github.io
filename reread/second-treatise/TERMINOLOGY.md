...
## 핵심 번역어
| English | Re:Read 우선 번역 | 운용 원칙 |
|---|---|---|
| state of nature | 자연 상태 | 정치사회 성립 이전의 규범 없는 상태로 오해하지 않도록 문맥 보존 |
| law of nature | 자연법 | reason과 연결될 때 논증 구조 유지 |
| natural right | 자연권 | 원문에 해당 표현이 있을 때 사용 |
| property | 재산 / 소유 / 생명·자유·재산 | 문맥에 따라 구분, 기계적 통일 금지 |
| consent | 동의 | 명시적 동의와 묵시적 동의를 구분 |
| express consent | 명시적 동의 | 사회의 완전한 구성원이 되겠다는 적극적 선언·계약 |
| tacit consent | 묵시적 동의 | 영토·토지·보호의 향유 기간에 법에 복종하는 동의이며 구성원 자격과 구별 |
| majority | 다수 / 다수의 결정 | 공동체가 하나의 정치체로 행동하게 하는 결정 원리 |
| body politic | 정치체 | 하나의 몸처럼 결합하여 다수의 의지로 행동하는 정치적 결합 |
| original compact | 최초의 계약 | 자유인들이 하나의 정치사회로 결합하는 원초적 합의 |
| democracy | 민주정 | 입법권과 집행권을 다수가 직접 행사하는 형태 |
| oligarchy | 과두정 | 입법권을 소수와 그 상속인·후계자에게 맡기는 형태 |
| monarchy | 군주정 | 입법권을 한 사람에게 맡기는 형태 |
| hereditary monarchy | 세습군주정 | 군주와 상속인에게 권력이 이어지는 형태 |
...
| political power | 정치권력 | 계약·합의·상호 동의에서 나오며 생명·자유·소유의 보존을 목적으로 함 |
| paternal power | 부권 | 미성년 자녀의 이익·교육·보존을 위한 자연적 권력 |
| parental power | 부모권 | 어머니와 아버지의 공동 권한을 드러내는 표현 |
| despotical power | 전제적 권력 | 정당한 전쟁에서 생명을 상실한 공격자 위에만 성립하는 절대적 권력 |
| forfeiture | 권리의 상실 | 침략자가 전쟁 상태에 들어가 자기 생명에 대한 권리를 잃는 경우 |
...
| community | 공동체 | 결합된 인민 전체의 집합적 주체 |

## 구분 원칙
- `commonwealth`는 민주정 같은 정부 형태가 아니라 모든 독립된 **정치공동체**를 뜻한다.
- `supreme power`는 제도상 최고인 입법권을 가리키더라도 인민의 동의·자연법·공공선·신탁의 한계를 넘지 못한다.
...
1. 원문의 논증 순서를 유지한다.
2. 한 절을 요약하지 않는다.
3. 긴 문장은 한국어 호흡에 맞게 나누되 같은 대응 블록 안에 둔다.
4. 현대 헌법 용어로 과도하게 치환하지 않는다.
5. 번역어가 달라질 때는 문맥상 이유를 검수 기록에 남긴다.